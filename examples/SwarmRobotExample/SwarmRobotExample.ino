...
enum RobotStatus {Ready_for_new_task=0,
	Heading_to_pickup=1,
	Picking_up=2,
...
public:
	...
	void event(float * buffer){
		...
		Serial.println("Pick Order Recived from : "+String(pickupArea)+" "+String(pickupX)+" "+String(pickupZ)+
				" to : "+String(dropOffArea)+" "+String(dropOffX)+" "+String(dropOffZ));
	}
};
...
public:
	...
	void event(float * buffer){
		...
		buffer[7]=100;// Y size
		buffer[8]=100;// Z size
		Serial.println("Location request "+String(buffer[0])+" "+String(buffer[1])+" "+String(buffer[2])+
				" "+String(buffer[3])+" "+String(buffer[4])+" "+String(buffer[5])+" "+String(buffer[6])+" "+String(buffer[7])+" "+String(buffer[8]));
	}
};
